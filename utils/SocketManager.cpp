...
SocketManager::VMPL::VMPL() {}

void SocketManager::VMPL::init()
{
    string ip[M] = {"127.0.0.1", "127.0.0.1"};
    int port[M] = {88888, 66666};
    init(ip, port);
}

void SocketManager::VMPL::init(string *ip, int *port)
{
    this->ip = ip;
    this->port = port;
    init_windows_socket();
...
void SocketManager::VMPL::server()
{
...
void SocketManager::VMPL::client()
{
    //    printf(" asdasd %d\n", port);
...
void SocketManager::VMPL::server_exit()
{
    socket_close(serv_sock);
    exit_windows_socket();
}

void SocketManager::VMPL::client_exit()
{
    for (int i = 0; i < M; i++)
    {
        if (i != party)
...
void SocketManager::VMPL::exit_all()
{
    server_exit();
    client_exit();
}

void SocketManager::VMPL::Send(MatrixXu *a)
{
    socket_io[party][!party]->send_message(a);
}

void SocketManager::VMPL::Receive(MatrixXu *a)
{
    socket_io[party][!party]->recv_message(*a);
}

void SocketManager::VMPL::Send(MatrixXb *b)
{
    socket_io[party][!party]->send_bits(b);
}

void SocketManager::VMPL::Receive(MatrixXb *b)
{
    socket_io[party][!party]->recv_bits(*b);
}

void SocketManager::VMPL::Send(MatrixXi *a)
{
    socket_io[party][!party]->send_message(a);
}

void SocketManager::VMPL::Receive(MatrixXi *a)
{
    socket_io[party][!party]->recv_message(*a);
}
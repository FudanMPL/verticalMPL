...
void SocketOnline::send_message(MatrixXi *a)
{
    // int len_buffer;
    int r = a->rows();
    int c = a->cols();
    send_message_n(sock, (char *)&r, 4);
    send_message_n(sock, (char *)&c, 4);
    send_message_n(sock, (char *)a->data(), r * c * 4);
}

void SocketOnline::recv_message(MatrixXi &a)
{
    int r, c;
    recv_message_n(sock, (char *)&r, 4);
    recv_message_n(sock, (char *)&c, 4);
    a.resize(r, c);
    recv_message_n(sock, (char *)a.data(), r * c * 4);
}

void SocketOnline::send_bits(MatrixXb *b)
{
    int r = b->rows();
    int c = b->cols();
    send_message_n(sock, (char *)&r, 4);
    send_message_n(sock, (char *)&c, 4);
    send_message_n(sock, (char *)b->data(), r * c);
}
...
void SocketOnline::recv_bits(MatrixXb &b)
{
    int r, c;
    recv_message_n(sock, (char *)&r, 4);
    recv_message_n(sock, (char *)&c, 4);
    b.resize(r, c);
    recv_message_n(sock, (char *)b.data(), r * c);
}
